...
describe("Scrape Job summary table", () => {
  it("uses the Primary Identifier Field Label and presents lightweight job details", () => {
    ...
    expect(within(table).getByRole("link", { name: "Customer 41" })).toHaveAttribute(
      "href",
  ...
  it("shows exact status counts and filters without changing API order", async () => {
    ...
    expect(screen.getByRole("option", { name: "Pending (1)" })).toBeInTheDocument()
    expect(screen.getByRole("option", { name: "In progress (0)" })).toBeInTheDocument()
    expect(screen.getByRole("option", { name: "Complete (1)" })).toBeInTheDocument()
    expect(screen.getByRole("option", { name: "Failed (2)" })).toBeInTheDocument()
    expect(screen.getByRole("option", { name: "Cancelled (0)" })).toBeInTheDocument()
    await userEvent.click(screen.getByRole("option", { name: "Failed (2)" }))

    const rows = within(screen.getByRole("table", { name: "Scrape Jobs" })).getAllByRole("row")
  ...
  it("paginates 25 rows, resets on filter changes, and clamps after polling shrinks the list", async () => {
    const completeJobs = Array.from({ length: 31 }, (_, index) => job(index + 1))
    const { rerender } = render(
      <ScrapeJobSummaryTable run={run(completeJobs)} />,
    )

    expect(screen.getByRole("link", { name: "Customer 1" })).toBeInTheDocument()
    expect(screen.getByRole("link", { name: "Customer 25" })).toBeInTheDocument()
    ...
    expect(screen.getByRole("link", { name: "Customer 26" })).toBeInTheDocument()
  ...
  ] as const)("shows the zero-job empty state for a %s Run", (status, message) => {
    render(
      <ScrapeJobSummaryTable
        run={run([], {
          status,
          finishedAt: status === "complete" ? "2026-04-01T10:10:00.000Z" : null,
        })}
      />,
    )
